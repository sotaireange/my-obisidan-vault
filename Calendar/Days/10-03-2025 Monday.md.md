...
```dataview
LIST
WHERE file.tags INCLUDES "#tasks"
FLATTEN file.frontmatter AS metadata
WHERE metadata.read OR metadata.video OR metadata.article OR metadata.leetcode
GROUP BY file.name
FLATTEN metadata.read AS read
FLATTEN metadata.video AS video
FLATTEN metadata.article AS article
FLATTEN metadata.leetcode AS leetcode
SORT file.name ASC
```
